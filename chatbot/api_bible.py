...
def get_versiculo(passage, biblia_version = "RVR60.js"):
    ...
    print("{0}|INFO | get_versiculo | {1}".format( datetime.utcnow().isoformat() , txt[0] ))
    return txt
#######################################################################################
def req_get(get_json , timeout=None):
    ...
    try:
        ...
        print("{0}|INFO | req_get | {1} | {2} | URL_API=[{3}]".format( datetime.utcnow().isoformat() , rpt.status_code, rpt.reason, URL_API[:40]) )
